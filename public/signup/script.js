...
if (localStorage.getItem('token') !== null) {
  axios({
    ...
    data : {token : localStorage.getItem('token')}
      ...
      .then(function() { window.location.replace('/upload') })
      .catch(function() { localStorage.removeItem('token') })
}

axios({method : 'get', url : '/api/info'})
    .then(function(response) { infoapi = response.data })
...
function login() {
  ...
  axios({method : 'post', url : '/api/user/signup', data : {dataObject}})
      .then(function() {
        Swal.fire({
      ...
      .catch(function(error) {
        ...
        $('#errormessage')
            .append(
                `<div id="errortext" style="margin-top: 5px;"><p class="tag is-danger">${
